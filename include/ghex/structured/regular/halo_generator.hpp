...
class halo_generator<DomainIdType, std::integral_constant<int, Dimension>>
{
  ...
  public: // ctors
    /** @brief construct a halo generator
      ...
      * @param include_corners indicates wether halos are considered include corners as well */
    ...
    halo_generator(const Array& g_first, const Array& g_last, RangeHalos&& halos,
        RangePeriodic&& periodic, bool include_corners = true)
    ...
    auto operator()(const domain_type& dom) const
    {
        ...
        if (m_include_corners) outer_halos = compute_spaces<box2>(outer_spaces);
        else
        ...
        {
            if (j == (ghex::util::ct_pow(3, dimension::value) / 2)) continue;
    ...
    box2 intersect(const domain_type& /*d*/, const coordinate_type&     first_a_local,
        const coordinate_type& /*last_a_local*/, const coordinate_type& first_a_global,
  ...
  private: // member functions
    ...
    {
        std::vector<Box> x;
        x.reserve(ghex::util::ct_pow(3, dimension::value));
        Box b;
    ...
    {
        if (d == dimension::value) { c.push_back(current_box); }
        else
        {

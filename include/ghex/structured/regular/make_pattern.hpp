...
{
    using pattern_type = ghex::pattern<typename detail::domain_traits<DomainRange>::grid_type,
        typename detail::domain_traits<DomainRange>::domain_id_type>;
    using pattern_container_type =
        ghex::pattern_container<typename detail::domain_traits<DomainRange>::grid_type,
            typename detail::domain_traits<DomainRange>::domain_id_type>;
    ...
    auto comm = ghex::mpi::communicator(ctxt);
        ...
        for (auto& p : my_patterns[i])
        {
            ...
            for (auto& id_is_pair : p.recv_halos())
            {
                ...
                if (it == tag_map.end())
                {
                    ...
                    tag_map[rank].push_back(
                        tag_item{id_is_pair.first.id, p.domain_id(), prev_max_tag});

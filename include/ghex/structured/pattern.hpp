...
template<typename CoordinateArrayType, typename DomainIdType>
class pattern<structured::detail::grid<CoordinateArrayType>, DomainIdType>
{
  public: // member types
    using grid_type = structured::detail::grid<CoordinateArrayType>;
    using this_type = pattern<grid_type, DomainIdType>;
    ...
    struct iteration_space
    {
      ...
      public: // ctors
        ...
        : _min(std::forward<A>(a))
        , _max(std::forward<B>(b))
        {
            if (!(a <= b))
  ...
  public: // static member function
    ...
    friend class pattern_container<grid_type, DomainIdType>;
...
struct make_pattern_impl<ghex::structured::detail::grid<CoordinateArrayType>>
{
    ...
    static auto apply(context& ctxt, HaloGenerator&& hgen, DomainRange&& d_range)
    {
        ...
        for (const auto& d : d_range)
        {
            ...
            my_domain_extents.push_back(iteration_space_pair{
            ...
            my_patterns.emplace_back(my_domain_extents.back(), my_domain_ids.back());
            ...
            for (const auto& halo : recv_halos)
            {
                ...
                for (unsigned int j = 0; j < domain_extents.size(); ++j)
                {
                    ...
                    for (unsigned int k = 0; k < extents_vec.size(); ++k)
                    {
                        ...
                        {
                            my_patterns[i].recv_halos()[domain_id].push_back(iteration_space_pair{
                                iteration_space{coordinate_type{x.local().first()},
        ...
        for (auto x : max_tags) m_max_tag = std::max(x, m_max_tag);

        // translate my receive halos to (remote) send halos
        // by a detour over the following nested map
        std::map<int, std::map<domain_id_type,
                          std::map<extended_domain_id_type, std::vector<iteration_space_pair>>>>
            send_halos_map;
        ...
        for (const auto& p : my_patterns)
        {
            ...
            for (const auto& id_is_pair : p.recv_halos())
            {
                ...
                for (unsigned int l = s; l < is_vec.size(); ++l)
                {
                    ...
                    is_vec[l].local().first() =
                        is_l.first() + (is_vec[l].global().first() - is_g.first());
                    is_vec[l].local().last() =
                        is_l.first() + (is_vec[l].global().last() - is_g.first());
                }
            }
        }
        ...
        if (it != send_halos_map.end())
        {
            ...
            for (const auto& p1 : it->second)
            {
                ...
                for (const auto& p2 : p1.second) p.send_halos().insert(p2);
            }
        ...
        for (int rank = 0; rank < world_size; ++rank)
        {
            if (rank == comm.rank())
            {
                ...
                if (num_ranks > 0)
                {
                    ...
                    for (const auto& p : send_halos_map) ranks.push_back(p.first);
                    ...
                    for (const auto& p : send_halos_map) num_domains.push_back(p.second.size());
                    int j = 0;
                    for (auto& nd : num_domains) comm.send(ranks[j++], 0, nd);
                    ...
                    for (const auto& p : send_halos_map)
                    {
                        ...
                        for (const auto& p1 : p.second) dom_ids.push_back(p1.first);
                        ...
                        for (const auto& p1 : p.second) num_pairs.push_back(p1.second.size());
                        ...
                        for (const auto& p1 : p.second)
                        {
                            ...
                            for (const auto& p2 : p1.second) my_dom_ids.push_back(p2.first);

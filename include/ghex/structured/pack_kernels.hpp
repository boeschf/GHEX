...
#endif
    static void
    pack(PackIterationSpace&& pack_is, void*)
    {
...
#endif
    static void
    unpack(UnPackIterationSpace&& unpack_is, void*)
    {
...
pack_kernel(PackIterationSpace pack_is, unsigned int num_elements)
{
    ...
    for (unsigned int i = 0; i < num_elements; ++i)
    {
        if (index * num_elements + i < pack_is.m_buffer_desc.m_size)
        {
            ...
            ::ghex::structured::detail::compute_coordinate<D>::template apply<Layout>(
...
struct serialization<ghex::gpu, LMap>
{
    ...
    static void unpack(UnPackIterationSpace&& unpack_is, void* arg)
    {
        ...
        const std::size_t num_blocks = (num_threads + block_dim - 1) / block_dim;
        unpack_kernel<LMap>
            <<<num_blocks, block_dim, 0, *stream_ptr>>>(unpack_is, elements_per_thread);
    }

...
class bulk_communication_object
{
  ...
  private: // member types
    ...
    struct field_container
    {
        ...
        // construct from field, pattern and the maps storing rma handles, and patterns
        field_container(/*communicator_type& comm, */
                context& ctxt,
                const Field& f, const pattern_type& pattern,
        ...
        , m_co{std::make_unique<co_type>(ctxt)}
        {
            auto& comm = m_co->communicator();
            // initialize the remote handle - this will effectively publish the rma pointers
            // will do nothing if already initialized
  ...
  public: // ctors
    bulk_communication_object(context& ctxt, bool use_node_local_co = false)
    : m_ctxt{ctxt}
  ...
  public:
    ...
    void add_field(buffer_info_type<Field> bi)
    {
        ...
        {
            f_cont.push_back(
                field_container<Field>(/*m_co->communicator()*/m_ctxt, bi.get_field(), bi.get_pattern_container(),

...
class communicator
{
  ...
  public:
    ...
    template<typename T>
    std::vector<std::vector<T>> all_gather_b(
        const std::vector<T>& payload, const std::vector<int>& sizes) const
    {
        ...
        GHEX_CHECK_MPI_RESULT(MPI_Allgatherv(&payload[0], payload.size() * sizeof(T), MPI_BYTE,
            &recvbuf[0], &recvcounts[0], &displs[0], MPI_BYTE, *this));

        c = 0;
        for (int i = 0; i < size(); ++i)
        {
            std::copy(&recvbuf[displs[i] / sizeof(T)], &recvbuf[displs[i] / sizeof(T)] + sizes[i],
                res[i].begin());
        }
        return res;
    }
    ...
    std::vector<T> all_gather_b(const T& payload) const
    {
        std::vector<T> res(size());
        GHEX_CHECK_MPI_RESULT(
            MPI_Allgather(&payload, sizeof(T), MPI_BYTE, &res[0], sizeof(T), MPI_BYTE, *this));
        return res;
    }

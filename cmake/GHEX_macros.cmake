...
# ---------------------------------------------------------------------
function(ghex_add_config_cond_define definition)

  # if(ARGN) ignores an argument "0"
  set(Args ${ARGN})
  list(LENGTH Args ArgsLen)
  if(ArgsLen GREATER 0)
    set_property(GLOBAL APPEND PROPERTY GHEX_CONFIG_COND_DEFINITIONS "${definition} ${ARGN}")
  else()
    set_property(GLOBAL APPEND PROPERTY GHEX_CONFIG_COND_DEFINITIONS "${definition}")
  endif()
...
# ---------------------------------------------------------------------
function(ghex_add_config_define_namespace)
  set(options)
  ...
  set(DEF_VAR GHEX_CONFIG_DEFINITIONS_${OPTION_NAMESPACE})
...
function(write_config_defines_file)
  ...
  if (${OPTION_NAMESPACE} STREQUAL "default")
    get_property(DEFINITIONS_VAR GLOBAL PROPERTY GHEX_CONFIG_DEFINITIONS)
    get_property(COND_DEFINITIONS_VAR GLOBAL PROPERTY GHEX_CONFIG_COND_DEFINITIONS)
  else()
    get_property(DEFINITIONS_VAR GLOBAL PROPERTY
      GHEX_CONFIG_DEFINITIONS_${OPTION_NAMESPACE})
  endif()
  ...
  if (NOT OPTION_TEMPLATE)
    ...
    set(PREAMBLE
      ...
      "\n"
      "#ifndef GHEX_CONFIG_${NAMESPACE_UPPER}_HPP\n"
      "#define GHEX_CONFIG_${NAMESPACE_UPPER}_HPP\n"
    )
...
# ---------------------------------------------------------------------
function(ghex_option option type description default)
  set(options ADVANCED)
  set(one_value_args CATEGORY)
  set(multi_value_args STRINGS)
  cmake_parse_arguments(GHEX_OPTION "${options}" "${one_value_args}" "${multi_value_args}" ${ARGN})

  if(NOT DEFINED ${option})
    set(${option} ${default} CACHE ${type} "${description}" FORCE)
    if(GHEX_OPTION_ADVANCED)
  ...
  if(GHEX_OPTION_STRINGS)
    if("${type}" STREQUAL "STRING")
      set_property(CACHE "${option}" PROPERTY STRINGS "${GHEX_OPTION_STRINGS}")
    else()
      message(FATAL_ERROR "ghex_option(): STRINGS can only be used if type is STRING !")
    endif()
  endif()

  set(_category "Generic")
  if(GHEX_OPTION_CATEGORY)
    set(_category "${GHEX_OPTION_CATEGORY}")
  endif()
